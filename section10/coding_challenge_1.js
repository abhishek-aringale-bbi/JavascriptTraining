...
1. Create a method called 'registerNewAnswer' on the 'poll' object. The method does 2 things:
  ...
  For example, if the option is 3, increase the value AT POSITION 
  3 of the array by 1. Make sure to check if the input is a 
  number and if the number makes sense (e.g answer 52 wouldn't 
  make sense, right?)
2. Call this method whenever the user clicks the "Answer poll" 
button.
3. Create a method 'displayResults' which displays the poll 
results. The method takes a string as an input (called 'type'), 
which can be either 'string' or 'array'. If type is 'array', 
simply display the results array as it is, using console.log(). 
This should be the default option. If type is 'string', display a
...
const poll = {
  ...
  registerNewAnswer() {
    ...
    if (ans === 0) {
      this.answers[0]++;
    } else if (ans === 1) this.answers[1]++;
    else if (ans === 2) this.answers[2]++;
    else if (ans === 3) this.answers[3]++;
